...
class ProjectService {
     ...
     */
    async getAllProjects(page: number = 1, limit: number = 10): Promise<ProjectsResponse> {
        try {
     ...
     */
    async updateProject(id: string, data: Partial<CreateProjectData>, images?: File[]): Promise<Project> {
        try {
            const formData = new FormData();
     ...
     */
    async getAllSkills(): Promise<Technology[]> {
        try {
     ...
     */
    extractPlainText(htmlContent: string, maxLength: number = 200): string {
        // Remove HTML tags
